## Author: Mengli Xiao, PhD

## install packages
# system requirement: R < 4.0.0
### "metafor" needs be installed with previous versions to make sure the repMeta package appropriately works.
packages <- c("evd", "parallel")
install.packages(setdiff(packages, rownames(installed.packages())))
### get your current directory for storing this example.R file
file.path <- rstudioapi::getActiveDocumentContext()$path
path <- dirname(file.path)
### install previous version (2.4-0) of "metafor" from CRAN archive and "repMeta" from source
packageurl <- "https://cran.r-project.org/src/contrib/Archive/metafor/metafor_2.4-0.tar.gz"
install.packages(packageurl, repos=NULL, type="source")
install.packages(paste0(path,"repMeta_0.1.0.tar.gz"), repos = NULL, type="source")

## load packages
library(metafor)
library(evd)
library(parallel)
library(repMeta)

## Transform the data
### The data should contain the columns with effect size named by "y" and within-study variance named by "s2"
data.case <- to.dat.repMeta(data=moller12,ai = r1, n1i = n1, ci = r2, n2i = n2,measure="OR")

## Calculate $R_1$ for the given data
calR(data.case,m=1)

## Replicability test
### Replicability test based on Gumbel approximation
pRmasym(data.case,m=1)

### Replicability test based on the bootstrap approximation
pRmboot(data.case,m=1)

## Identify studies with non-replicable results
### Identify studies using Gumbel approximation
Rm.func.iterative(data.case,m=1)

### identify studies using bootstrap approximation
Rm.func.iterative.boot(data.case,m=1)
